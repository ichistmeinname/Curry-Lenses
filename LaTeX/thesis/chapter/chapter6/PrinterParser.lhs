...
Pretty-printers and parsers are well-studied fields in computer
science and take an important role in the design of programming
languages. %
...
In order to analyse these results, we read the CSV data from the
files, manipulate them and write the results back to another file. %
Whereas parsers specify the format of the CSV data at hand and yield
an editable structure in the source language, printers write the
results back in a valid format. %
We observe that printers and parsers should fulfil certain
round-tripping rules: we want to print the data in a valid format,
such that data can be reread again by the parser. %
...
definition of pretty-printers and parsers into one function. %
We can achieve this combination in the setting of lenses: we define a
lens for pretty-printing in the put and parsing in get direction. %
...
expressions. %
We use the following definition of an algebraic data type for
...
|pParse| chooses the first pair of the resulting list that has no
remaining string, i.e., a result with a complete parse, and yields the
parsed value. %
...
We ignore the given string, replace it with the representation of the
given digit, and add the remaining string at the end of the resulting
string. %
Since the type of the data is not restricted to digits only, we
include a check if the range of the given integer value is between |0|
and |9|. %
...
paired with the remaining string. %
...
In order to test this lens definition, we need to define the
...
operator |Plus|. %
Because of this restriction, we cannot, however, parse a numeric value
like |"42"|. %
Hence, the last expression |pParse ppExpr "42"| fails with the remark
that the given string cannot be parsed completely. %
Without investigating this failing expression in too much detail, we
can take a quick look at the parsing result. %
...
we have to rewrite the second rule of |ppExpr|'s definition. %
...
In particular, the spaces that we added are ignored in
...
In the case of an injected prettiness factor like additional spaces,
it would be convenient to provide a combinator to ignore a
printer-parser's result. %
Those additional adjustments regarding the printed string occur mostly
...
In the forth expression, we reuse the example from above that has a
number with two digits as second argument for |Plus|, thus, leading to
a remaining string. %
Note, the parse does not fail completely: the prefix |"+ 2 3"| is
parsed correctly and the parser yields |BinOp Plus (Num 2) (Num 3)| as
...
If these redundancies do not appear in the pretty-printer's
definition, we do have no option to parse them anyway. %
We have already mentioned an original precedent in the introduction of
this section: optional spaces as delimiter between tokens. %
...
The function |spaces1'| pretty-prints a series of spaces depending on
the length of the given list, which cannot be empty. %
In contrast, the auxiliary function |spaces'| can pretty-print zero or
many spaces. %
For the parsing direction, we want both functions to parse a series of
spaces. %
A generalisation of these functions comes in handy to parse a series
of characters or strings of the same category and to pretty-print a
...
printer-parser for arithmetic expressions. %
...
Possible versions include a different number of trailing space after
the operator and the first argument of that operator. %
This unsatisfactory result arises from the use of the free variables
in the definition of |ppExprSpaces|. %
We cannot set the number of used spaces to one; the free variable is
instantiated nondeterministically to a suitable value. %
...
our problem. %
They introduce \emph{quotient lenses}, well-behaved bidirectional
transformations that allow the programmer to specify equivalence
relations on the data he wants process. %
The authors integrated an implementation of quotient lenses to the
Boomerang language. %
However, we did not have the time to implement a mature version of our
own quotient lenses in the context of our study case. %

As a second disadvantage, the definitions for the pretty-printer have
to be more sophisticated than usual. %
...
In our example, we defined a string representation for arithmetic
expression in prefix notation, but with infix operators, we have to
avoid left-recursion in the definitions. %
This modification leads to a more complex definition of the
printer-parsers that bears a resemblance to a typical parser for
...
These disadvantages arise from the operational, state-free approach of
...
In the previous subsection, we gave an unsatisfactory definition to
parse a variable amount of spaces. %
...
of the underlying parser without changing the semantics of the
pretty-printer. %
...
Because of the changed data structure, we can also adjust the semantics
and add an inspection of the given input string. %
Instead of replacing the input string completely, we want to perform
a layout preserving replacement. %
The new implementation provides a function |replaceParse :: PReplace a
-> RPLens a| that takes a specification of a so-called
...
The result of our replace-parser assures that the layout of the given
string is preserved. %
On the other hand, we can parse the resulting string with the
replace-parser again and, hopefully, regain the original value of the
arithmetic expression. %
...
implementation. %
At first, we want to discuss the used data structure that tracks the
...
|RPLens| that wraps the resulting string into a new data structure. %
...
In order to give an example, we get the following results for a
primitive combinator |charP :: (Char -> Bool) -> PReplace ()| based on
the given string. %
...
In these examples, we want to replace a digit character with |'5'|. %
The first example succeeds with a replacement, because the given string
consists of a digit as well. %
In the second and third exemplary expression, we ignore the input,
because it does not harmonise with the given specification and
...
We replace the input and pretty-print the value for two cases: if the
input string is empty, or if the input does not fulfil the given
predicate. %
...
charP p input (v,new) = case  input of
  ...
  where
   char' (c':str') rest
     | p c' && (null str' || rest == str')  = Replaced rest
...
That is, if we want to replace a digit with another digit and the
input string consists of two digits, the specification is not
fulfilled and we pretty-print the value. %
However, for an identical remaining string, the replace-parser can
perform a replacement, indicated by the usage of the |Replaced|
constructor. %
...
primitives. %
In the examples, we used |charP| to replace and pretty-print a digit;
...
greater than |9| is converted into a character. %
...
% one. %
% As a consequence, we always use both parsers nondeterministically in
% the parsing direction. %
% When we parse a string, the input string is not empty, thus,
...
its corresponding value and its part of the input. %
As remaining string, we provide the result of the second
replace-parser that works on its part of the input string and the
given remaining string. %
That is, we concatenate the results of both parsers and add the
remaining string at the end. %
...
strict pReplace str pair = case  pReplace str pair of
                                 New _ -> failed
                                 res   -> res
\end{spec}
...
Thus, these primitives yield a string wrapped in a |New|
constructor. %
In the case of an empty input string, we apply both replace-parsers in
series without using the strict version. %

Furthermore, the implementation of |(<*)| and |(*>)| is
straightforward, and the same as for the printer-parser, but with an
adapted type signature. %
...
\subsubsection*{Arithmetic Expressions Revisited}

In order to compare this approach with our previous implementation of
printer-parsers, we define a replace-parser for arithmetic
expressions. %
...
hence, leading to a failing parsing action on every input string. %
...
The more interesting part is the behaviour of our given implementation
for replace-parsers. %
...
Nonetheless, we want to give some more examples to highlight the
improvement in contrast to the previous implementation. %
...
We introduce an additional combinator, |(<*)|, that splits the input
string into two parts for each space. %
The longer the input string and the more composition
...
We also measured the performance for a parsing action and,
unfortunately, the results are even worse. %
The results are illustrated with a logarithmic scale in
Figure~\ref{fig:plotParseB}; the runtime of a parsing action behaves
highly exponential as well. %
...
\begin{figure}[h!]
  ...
  \caption{Performance of replacing for increasing number of
    combinators}
...
\begin{figure}[h!]
  ...
  \caption{Performance of replacing for increasing number of
    combinators with logarithmic scale}
...
\end{spec}
%
The expression consists of thirty combinators, has three levels of
...
Instead of a functional-logic approach that makes heavy use of
...
predicate holds, and remember the remaining input in the second
component. %
...
string of the first replace-parser as argument for the second one. %
...
However, there is one exception: if the given replace-parser does not
care about its input, we do not want the composition to fail. %
Therefore, we use |pure| -- a primitive replace-parser that succeeds on
every input -- to handle the special case of a non-consuming
replace-parser. %
...
This combination allows us to consume an arbitrary number of spaces,
and stop whenever the |space| parser fails. %
We also stop if the input string becomes empty, and yield a result to
express a successful consumption. %
...
that the expected runtime with respect to the number of used
...
Several papers dealing with bidirectional programming mention the
...
which the authors illustrate a so-called megamodel of parsing. %
This megamodel includes all different artefacts, and the corresponding
mappings of one artefact to another. %
As a result of this investigation, the authors apply different
...
propose a new interface to describe parsers and pretty-printers in a
single program. %
They provide a type class |Syntax delta| that includes common parser
functions like |empty|, |pure|, |(<$$>)|, |(<*>)|, and |(<||>)|. %
Due to the usage of pretty-printers, the composition combinator,
|(<*>)|, forms a pair instead of using the
traditional composition semantic of parsers -- like in our implementation. %
...
This approach has similarities to using a pair of functions to
...
FlipPr produces a parser for a context free grammar that is consistent
with the given definition of the pretty-printer. %
Their advantage in comparison to bidirectional approaches is that they
can outsource their parsing algorithms to parser generators and reuse
efficient implementations of exisiting pretty-printer libraries. %
...
\emph{happy}\footnote{\url{http://www.haskell.org/happy/}} as the
parser generator. %
