...
are lenses. %
Instead of an one-to-one mapping, lenses provide a component to
project and update a given source. %
...
describes the reversed situation, but needs an additional source as
argument. %
...
function, and then derive the corresponding put function from that
definition. %
...
like Has-kell, or define their own functional language that fits a
...
definitions forwards and backwards. %
...
In this chapter, we give a more detailed introduction of
...
related work that uses bidirectionalisation techniques. %
...
Last but not least, we discuss emerging challenges with bidirectional
programming in Curry and conclude in Chapter~\ref{ch:conclusion}. %
